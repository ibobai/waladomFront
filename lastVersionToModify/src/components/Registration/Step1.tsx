...
const Step1: React.FC<Step1Props> = ({ handleNext }) => {
  ...
  const validateEmail = async (email: string) => {
    try {
      const response = await fetch(
        "https://www.waladom.club/api/user/email/validate",
        {
          method: "POST",
          headers: {
  ...
  const sendVerificationEmail = async (email: string) => {
    try {
      const response = await fetch(
        "https://www.waladom.club/api/verification/email/send",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : t("registration.generalError")
      );
    } finally {
      setIsLoading(false);
    }
